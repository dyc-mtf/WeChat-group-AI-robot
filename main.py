...
with open("ApiKey_Data\AI_api_key.txt","r",encoding="utf-8") as f:
    deepseek_api_key = f.read()  # 本地ollama可能不需要API key，这里保留配置文件读取但实际不使用
...
def auto_send_task(): #让机器人主动发送消息的函数
    ...
    if enable_robot_send:
        ...
        if time_dist >= robot_send_time:
            ...
            deepseek_return = ollama_api_use(AI_msg)  # 修改为调用ollama接口
...
def ollama_api_use(user_input):  # 修改为ollama API调用
    ...
    payload = {
        "model": "deepseek-r1:8b",  # 本地部署的模型名称
...
def on_message(msg, chat): #比较重要的信息模块，主要作用是接收消息并发送给AI得到回复，然后再发送给群里
    global emotion_cnt, emoji_wait_time, enable_emotion, temp_content, robot_send_time, receiver_name
    global enable_robot_send, start_h, start_min, start_emoji_wait, end_emoji_wait, deepseek_return, robot_name
    global AI_msg, msg_cnt, msg_send_wait, now_msg_time, last_msg_time
    ...
    if isinstance(msg, FriendMessage):
        ...
        if msg_cnt >= msg_send_wait: # 如果消息量大于等于接受范围，就把消息合起来发给AI
            msg_cnt = 0
            #msg_send_wait = random.randint(start_msg_wait, end_msg_wait)
            print("消息列表:\n" + temp_content)

            deepseek_return = ollama_api_use(temp_content)  # 改为调用ollama接口
            temp_content = ""
        ...
        if not all(num == 0 for num in last_msg_time):
            ...
            if (average_cnt >= (msg_send_wait - 1) and dist_start) or (
                ...
                # last_msg_time = [0 for _ in last_msg_time]
                average_cnt = 0
